# Publish Lakeside-mem to GitHub

## Primary marketing hook (use this first line everywhere)

`Lakeside-mem cold-starts project grounding in ~422ms (first index build) / ~163ms warm, while cutting onboarding context by 94.71% vs a curated full-text pack (local benchmark).`
...
- A GitHub repo: `<YOUR_ORG_OR_USER>/Lakeside-mem`
- GitHub CLI authenticated (`gh auth login`) or browser-authenticated git credentials

## 1) Create repo (choose one)

### Option A: Web UI
Create `https://github.com/new` with name `Lakeside-mem` under your org/user account.

### Option B: GitHub CLI

```bash
gh repo create Lakeside-mem --public --source=. --remote=origin --push=false
```

## 2) Push current local branch

```bash
cd /ABS/PATH/Lakeside-mem
git remote set-url origin https://github.com/<YOUR_ORG_OR_USER>/Lakeside-mem.git
...
# https://github.com/<YOUR_ORG_OR_USER>/Lakeside-mem/compare/main...<your-branch>
```
...
- Keep skill files in `Skills/Lakeside-mem/`
