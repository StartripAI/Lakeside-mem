...
This guide describes production-style operation of `Lakeside-mem` in IDE/CLI workflows.
...
- `Scripts/lakeside_mem.sh`: shell wrapper
- `Scripts/repo_knowledge.py`: code retrieval engine used by `ask`
- `Skills/lakeside-mem/`: Codex skill package
...
Hard rules:
- do not bypass `lakeside_mem.sh run-target` in cross-repo runs
...
```bash
bash Scripts/lakeside_mem.sh init --project my-project
```
...
```bash
bash Scripts/lakeside_mem.sh session-start s100 --project my-project --title "Refactor stream pipeline"
bash Scripts/lakeside_mem.sh prompt s100 "Map current bottlenecks and propose safe migration order" --project my-project
bash Scripts/lakeside_mem.sh tool s100 shell "rg -n 'stream' App" --project my-project --compact
bash Scripts/lakeside_mem.sh stop s100 --project my-project --content "checkpoint after search"
bash Scripts/lakeside_mem.sh session-end s100 --project my-project
```
...
```bash
bash Scripts/lakeside_mem.sh search "streaming orchestration" --project my-project --limit 20
```

### Layer 1 (NL): mem-search

```bash
bash Scripts/lakeside_mem.sh mem-search "what bugs were fixed this week" --project my-project --limit 20
```

### Layer 2: timeline neighborhood

```bash
bash Scripts/lakeside_mem.sh timeline E42 --before 5 --after 5
```

### Layer 3: full details

```bash
bash Scripts/lakeside_mem.sh get E42 O8 O9
```

## 6) Fused Retrieval (Memory + Repo)

```bash
bash Scripts/lakeside_mem.sh ask "What is the stream update chain from input to persisted output?" --project my-project
```
...
```bash
bash Scripts/lakeside_mem.sh run-target "/ABS/PATH/TO/TARGET_PROJECT" \
  --project target \
  --executor codex \
  --question "learn this project: architecture, persistence, risks" \
...
```bash
bash Scripts/lakeside_mem.sh tool s100 shell "credential=<REDACTED_VALUE>" --project my-project \
...
```bash
bash Scripts/lakeside_mem.sh web --project-default my-project --host 127.0.0.1 --port 37777
```
...
```bash
codex mcp add lakeside-mem -- python3 /ABS/PATH/TO/lakeside-mem/Scripts/codex_mem_mcp.py --root /ABS/PATH/TO/lakeside-mem --project-default my-project
```
...
```bash
bash Scripts/lakeside_mem.sh export-session s100 --anonymize on --output /tmp/s100_export.json
```
