...
This playbook is for short user prompts, but with strict operating rules. `Lakeside-mem` handles routing and prompt shaping internally; users should not send long template prompts.
...
No workflow is considered valid if it does not run through the `Lakeside-mem` entrypoint with an explicit target root.

## Mandatory Execution Entrypoint (Hard Requirement)

Always execute via `lakeside_mem.sh run-target` with both:
...
- bypassing `lakeside_mem.sh run-target` in cross-repo onboarding flows
...
2. Else use current IDE/CLI workspace root if it is not the `Lakeside-mem` repo.
...
Only callable prompts are allowed. Any prompt that cannot trigger `Lakeside-mem` is invalid.
...
4. One fully executable next `ask` command.
   - Must use `lakeside_mem.sh run-target "/ABS/PATH/TO/TARGET_PROJECT"` format.
