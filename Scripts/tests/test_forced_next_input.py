...
class ForcedNextInputTests(unittest.TestCase):
    def test_forced_output_includes_command_and_required_fields(self) -> None:
        ...
        self.assertIn("缺失章节", str(status_policy.get("status_query_rule_zh", "")))
        ...
        self.assertIn("当前环境支持执行", str(nxt.get("callable_prompt_zh", "")))
        self.assertIn("当前环境不支持执行", str(nxt.get("callable_prompt_zh", "")))
        self.assertIn("用户追问完成度时", str(nxt.get("callable_prompt_zh", "")))
        self.assertIn("95%+", str(nxt.get("callable_prompt_zh", "")))
        self.assertIn("每项至少 3 条", str(nxt.get("callable_prompt_zh", "")))
