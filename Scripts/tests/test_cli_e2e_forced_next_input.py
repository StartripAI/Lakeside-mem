...
class CliE2EForcedNextInputTests(unittest.TestCase):
    def test_cli_ask_always_returns_forced_next_input(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="codex_mem_e2e_") as tmp:
            ...
            self.assertIn("当前环境支持执行", str(next_input.get("callable_prompt_zh", "")))
            self.assertIn("当前环境不支持执行", str(next_input.get("callable_prompt_zh", "")))
            self.assertIn("用户追问完成度时", str(next_input.get("callable_prompt_zh", "")))
            self.assertIn("95%+", str(next_input.get("callable_prompt_zh", "")))
