...
<head>
  ...
  <title>Lakeside-mem viewer</title>
  <style>
    :root {
      --bg: #f5f7fb;
      --panel: #ffffff;
...
<body>
  <div class="panel">
    <h1>Lakeside-mem local viewer</h1>
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if path == "/":
                ...
                return
            if path == "/api/health":
                self._json({"ok": True, "service": "lakeside-mem-viewer"})
                return
            if path == "/api/config":
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Lakeside-mem local web viewer")
