...
`codex-mem` is a memory runtime for coding workflows.
...
Use this when you want to run `codex-mem` against another repository.
...
bash Scripts/codex_mem.sh prompt s1 "Map generation and persistence path" --project demo
bash Scripts/codex_mem.sh tool s1 shell "rg -n 'DatabaseBootstrapper|index.ts'" --project demo --title "Locate chain" --compact
...
bash Scripts/codex_mem.sh search "persistence bootstrap" --project demo --limit 20

# Layer 2: timeline around one item
bash Scripts/codex_mem.sh timeline E12 --project demo --before 5 --after 5

# Layer 3: full details
bash Scripts/codex_mem.sh get E12 O3 --project demo
```

### 3) Run fused ask

```bash
bash Scripts/codex_mem.sh ask "map entrypoint, ai generation path, persistence, and top risks" --project demo
```
...
- `Scripts/codex_mem.sh`: shell entrypoints including `run-target` and `run-target-auto`.
